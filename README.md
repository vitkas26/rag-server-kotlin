...
Прод-инстанс сейчас не поднят: VPS приостановлен (оплата не продлена), поэтому этот workflow не
отработает, пока сервис не запущен заново. Деплой-скрипт (`deploy_vps.sh`) и конфигурация
актуальны — сервис поднимается им же при необходимости.
