...
const val CODE_TABLE = "code_chunks"

class CodeContextRagAdapter(
    ...
    override suspend fun search(query: String): List<DocChunk> {
        val queryEmbedding = embeddingService.embedQuery(query).getOrElse { e ->
            throw RagError.EmbeddingError("Code query embedding failed: ${e.message}", e)
        }
        return repo.searchChunksWithScore(queryEmbedding, CODE_TABLE, config.rag.topK)
